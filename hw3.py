...
def lin_comb_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    m2cd = utils.mat2coldict(M)
    return sum([v.f[k]*m2cd[k] if k in m2cd else 0 for k in v.f])


## Problem 12
def lin_comb_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    m2rd = utils.mat2rowdict (M)
    return sum([v.f[k]*m2rd[k] if k in m2rd else 0 for k in v.f])
...
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    rowsA = utils.mat2rowdict (A)
    return utils.rowdict2mat({k:rowsA[k]*B for k in rowsA})
