...
def is_independent(L):
    ...
    if len(L) == 1:
        return True
    result = [is_superfluous (L, i) for i in range (len(L))]
...
def superset_basis(S, L):
    ...
    Example:
        ...
        True
    '''
    T = S[:] # make a copy of the linear independent vectors as the starting point and grow
    for v in L:
        if v in T: continue
        T.append(v)
        if not is_independent (T): T.remove(v)     
    return T
...
def exchange(S, A, z):
    ...
    Example:
        ...
        True
    '''
    SminusA = [v for v in S if v not in A]
    for i in range (len(SminusA)):
        M = [z] + A + SminusA[:i] + SminusA[i+1:]
        u = vec2rep (M, SminusA[i])
        if u is not None: return SminusA[i]
    return None